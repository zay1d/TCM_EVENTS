...
Подключите репозиторий к Cloudflare Pages, корень фронта — `index.html`.
В настройках фронта укажите базовый URL API (`https://api.ВАШ-ДОМЕН`).
(Конфигурацию API-URL во фронте добавим на этапе подключения фронта к бэкенду.)
